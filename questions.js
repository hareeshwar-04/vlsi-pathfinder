...
export const questions = [
  ...
  {
    id: 2,
    title: "The Invisible Power Drain",
    ...
    options: [
      { key: "lp", text: "Design 'master switches' that completely cut off power to idle sections of the chip — like turning off lights in empty rooms of a building." },
      ...
      { key: "dm", text: "Study why electrons 'leak' through the tiny gates at the atomic level, and build equations that predict exactly how much power is wasted." }
    ]
  },
  {
    id: 3,
    ...
    options: [
      { key: "lp", text: "Writing rules that define when each section of a large chip should turn on, sleep, or wake up — like programming a smart building's power schedule." },
  ...
  },
  {
    ...
    options: [
      { key: "lp", text: "Signals got scrambled when passing between two chip sections running at different voltages — the 'translator' circuit between them seems broken." },
  ...
  },
  {
    ...
    options: [
      { key: "lp", text: "Using the new transistors to create separate zones on the chip — fast ones where speed matters, energy-efficient ones elsewhere — to get the best of both worlds." },
  ...
  },
  {
    ...
    options: [
      { key: "lp", text: "Create small 'always-on' islands on the chip that keep essential info alive on minimal power, while shutting down everything else completely." },
  ...
  },
  {
    ...
    options: [
      ...
      { key: "md", text: "Designing the on-chip storage that feeds pixel data to the display — specialized high-bandwidth memory that can handle massive video data streams simultaneously." },
  ...
  },
  {
    ...
    options: [
      { key: "lp", text: "Design 'voltage translator' circuits that convert signals when they cross between chip sections running at different power levels, preventing data corruption." },
      { key: "md", text: "Create delicate sensing circuits that can detect incredibly tiny voltage differences inside storage arrays, correctly distinguishing stored 0s from 1s." },
  ...
  },
  {
    ...
    options: [
      { key: "lp", text: "Making sure the chip's power-up and power-down sequences work perfectly — ensuring it never freezes in a half-on, half-off state during boot or shutdown." },
  ...
  {
    id: 10,
    title: "Firmware Storage Upgrade",
    ...
    options: [
      { key: "lp", text: "Designing the chip so the firmware block only receives power during system startup, then shuts down completely to save energy during normal operation." },
  ...
  },
  {
    ...
    options: [
      { key: "lp", text: "The big picture: deciding which blocks of a large chip share power lines, when each block sleeps or wakes, and coordinating it all — like managing a city's power grid." },
      { key: "md", text: "The mid-level: designing grids of storage cells, the wiring that connects them, and the peripheral circuits that manage data flow — like architecting a high-tech automated library." },
  ...
  },
  {
    ...
    options: [
      { key: "lp", text: "The power management brain — it decides which AI processing clusters are active, which are asleep, and which are fully off, to keep total energy consumption under budget." },
  ...
  },
  {
    ...
    options: [
      { key: "lp", text: "Verifying the startup sequence — making sure different sections power on in the right order and no section receives garbled signals while its neighbors are still booting." },
      { key: "md", text: "Checking that the storage can reliably read and write under worst-case conditions — testing timing margins, noise tolerance, and data integrity across millions of cells." },
  ...
  },
  {
    ...
    options: [
      { key: "lp", text: "You design the master energy plan for entire chips — deciding which parts sleep, wake, and at what voltages — making devices last all day on a single charge." },
